...
public class BoardService {
	...
	public String pagination(RangeDTO rDTO) {
		...
		int pageNumber=3;
		//2. 화면에 보여줄 시작페이지qjsgh .1,2,3=> 1로 시작, 4,5,6=> 4, 7.8.9=>7
		int startPage=((rDTO.getCurrentPage()-1)/pageNumber)*pageNumber+1;
		//3.화면에 보여줄 마지막 페이지 1,2,3=>3
		int endPage=(((startPage-1)+pageNumber)/pageNumber)*pageNumber;
		//4.총페이지 수가 연산된 마지막 페이지수보다 작다면 총페이지 수가 마지막페이지수로 설정
		//456인 경우 4로 설정
		if(rDTO.getTotalPage()<=endPage) {
			endPage=rDTO.getTotalPage();
		}//end if
		//5.첫페이지가 인덱스 화면이 아닌 경우
		int movePage=0;
		...
		if(rDTO.getTotalPage() > endPage) { //뒤에 페이지가 더있음
			movePage=endPage+1;
